...
def run_backtest_analysis():
    ...
    with top_r:
        st.info(f"💡 Analyzing {selected_date.strftime('%B %d, %Y')} using previous 21 trading days (excluding selected date)")
    ...
    if all_spike_msgs:
        ...
        if all_rows_have_data:
            st.info(f"ℹ️ No volume spikes detected on {date_str} with current threshold settings.")
...
if run_backtest or "backtest_run" in st.session_state:
    ...
    with st.expander("📖 How to Use", expanded=True):
        ...
        - Detects spikes when volume exceeds threshold (avg × multiplier)
        """)
