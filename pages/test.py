...
# Show bucket selection only for 15m mode. In 4h mode bucket is fixed at 4h.
if st.session_state.candle_size == "15 min":
    ...
    st.sidebar.caption("🕒 Time Bucket: 4 hours (fixed in 4h mode)")
...
def render_card(name, rows, bucket_minutes, summary):
    ...
    if summary:
        chips = [
            f'<span class="badge neutral">Bucket: {bucket_lbl}</span>',
...
def run_volume_check():
    ...
    with top_l:
        ...
        st.markdown(
            ...
            f'<span class="badge neutral">Bucket: {"4h" if bucket_minutes==240 else str(bucket_minutes)+"m"}</span>'
    ...
    if all_spike_msgs:
        ...
        alert_msg = f"⚡ Volume Spike Alert — {format_bucket_label(bucket_minutes)} bucket\n\n" + "\n\n".join(formatted_msgs)
        print(alert_msg)
